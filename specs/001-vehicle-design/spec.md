...
1. **Given** a seed and no constraints, **When** a vehicle is generated, **Then** a complete legal
   vehicle is described, identified by its tech level and type rather than by a rolled name.
...
- **FR-020**: Divergence documentation MUST live on a documentation page of its own, listing every
  divergence by vehicle and figure with the published value, the value cetools produces, and the
  reason.
- **FR-020a**: The existing documentation check MUST be extended to cover that page, so a divergence
  whose prose no longer matches what cetools produces fails the quality gate rather than shipping.
  The check's own description in the maintained docs MUST be updated to name the new page.

#### Catalog

- **FR-021**: Fifteen published vehicles MUST ship as authored design files: three aircraft, six
  grav vehicles, five ground vehicles and the tunnel boring machine.
...
- **Vehicle**: The built result. Carries the design's choices plus everything derived from them —
  consumed and remaining spaces, performance codes, crew and passenger capacity, cargo, total price,
  build time — and is what the description paragraph is rendered from.
...
- **Catalog Entry**: One of the fifteen shipped design files, paired with the published stat block
  it re-derives.
...
- **SC-001**: All fifteen catalog designs build successfully.
- **SC-002**: Across the fifteen, every published figure either matches cetools' output exactly or
  ...
  documentation check rather than by review.
...
- **No names.** There is no vehicle names module and no rolled names.
...
- The existing documentation check, which must be extended to cover a documentation page it does not
  currently know about (FR-020a). This is the one place the feature reaches outside the vehicles
  domain and the CLI, and it changes the quality gate, so it is worth a reviewer's attention.
