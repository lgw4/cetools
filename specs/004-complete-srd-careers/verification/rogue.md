<!-- Open Game Content per OGL 1.0a; see LICENSE-OGL.txt -->

# Verification: Rogue

Source-first re-read against https://evolvedexperiment.github.io/cepheus-srd/character-creation.html,
independent of this feature's transcription pass, per spec.md FR-023.

| Field | Source | Committed (`rogue.toml`) | Verdict |
|---|---|---|---|
| Display name | Rogue | Rogue | match |
| Qualification | Dex 5+ | DEX 5+ | match |
| Survival | Dex 4+ | DEX 4+ | match |
| Commission | Str 6+ | STR 6+ | match |
| Advancement (promotion) | Int 7+ | INT 7+ | match |
| Re-enlistment | 4+ | 4+ | match |
| Advanced-education gate | Education 8+ (FR-004 uniform rule) | EDU 8+ | match |
| Medical tier | fringe (0%/50%/75%) | fringe | match |
| Always-available | not stated | (absent) | match |
| Re-enterable | not stated | (absent) | match |
| Personal 1 | +1 Str | STR +1 | match |
| Personal 2 | +1 Dex | DEX +1 | match |
| Personal 3 | +1 End | END +1 | match |
| Personal 4 | Melee Combat | Melee Combat | match |
| Personal 5 | Bribery | Bribery | match |
| Personal 6 | Gambling | Gambling | match |
| Service 1 | Streetwise | Streetwise | match |
| Service 2 | Mechanics | Mechanics | match |
| Service 3 | Gun Combat | Gun Combat | match |
| Service 4 | Melee Combat | Melee Combat | match |
| Service 5 | Recon | Recon | match |
| Service 6 | Vehicle | Vehicle | match |
| Specialist 1 | Computer | Computer | match |
| Specialist 2 | Electronics | Electronics | match |
| Specialist 3 | Bribery | Bribery | match |
| Specialist 4 | Broker | Broker | match |
| Specialist 5 | Recon | Recon | match |
| Specialist 6 | Vehicle | Vehicle | match |
| Adv. Education 1 | Computer | Computer | match |
| Adv. Education 2 | Gravitics | Gravitics | match |
| Adv. Education 3 | Jack o' Trades | Jack-of-All-Trades | match (canonical form, FR-017a normalization) |
| Adv. Education 4 | Medicine | Medicine | match |
| Adv. Education 5 | Advocate | Advocate | match |
| Adv. Education 6 | Tactics | Tactics | match |
| Rank 0 title | Independent | Independent | match |
| Rank 0 grant | Streetwise-1 | Streetwise 1 | match |
| Rank 1 title | Associate | Associate | match |
| Rank 1 grant | (none) | (absent) | match |
| Rank 2 title | Soldier | Soldier | match |
| Rank 2 grant | Gun Combat-1 | Gun Combat 1 | match |
| Rank 3 title | Lieutenant | Lieutenant | match |
| Rank 3 grant | (none) | (absent) | match |
| Rank 4 title | Underboss | Underboss | match |
| Rank 4 grant | (none) | (absent) | match |
| Rank 5 title | Consigliere | Consigliere | match |
| Rank 5 grant | (none) | (absent) | match |
| Rank 6 title | Boss | Boss | match |
| Rank 6 grant | (none) | (absent) | match |
| Cash row 1 | 1000 | 1000 | match |
| Cash row 2 | 5000 | 5000 | match |
| Cash row 3 | 5000 | 5000 | match |
| Cash row 4 | 5000 | 5000 | match |
| Cash row 5 | 10000 | 10000 | match |
| Cash row 6 | 20000 | 20000 | match |
| Cash row 7 | 50000 | 50000 | match |
| Material 1 | Low Passage | Low Passage | match |
| Material 2 | +1 Int | INT +1 | match |
| Material 3 | Weapon | Weapon | match |
| Material 4 | Mid Passage | Mid Passage | match |
| Material 5 | Weapon | Weapon | match |
| Material 6 | High Passage | High Passage | match |
| Material 7 | +1 Soc | SOC +1 | match |

## Notes

Single full-table fetch returned every field consistently with no ambiguity requiring a
cross-check. No corrections or deviations found for this career.
