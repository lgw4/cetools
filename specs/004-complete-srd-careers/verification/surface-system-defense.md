<!-- Open Game Content per OGL 1.0a; see LICENSE-OGL.txt -->

# Verification: Surface System Defense

Source-first re-read against https://evolvedexperiment.github.io/cepheus-srd/character-creation.html,
independent of this feature's transcription pass, per spec.md FR-023.

| Field | Source | Committed (`surface-system-defense.toml`) | Verdict |
|---|---|---|---|
| Display name | Surface Defense (source's own table header); the career-descriptions list gives the long form "Surface System Defense" (research.md R2/FR-005) | Surface System Defense | match |
| Qualification | End 5+ | END 5+ | match |
| Survival | Edu 5+ | EDU 5+ | match |
| Commission | End 6+ | END 6+ | match |
| Advancement (promotion) | Edu 7+ | EDU 7+ | match |
| Re-enlistment | 5+ | 5+ | match |
| Advanced-education gate | Education 8+ (FR-004 uniform rule) | EDU 8+ | match |
| Medical tier | service (75%/100%) | service | match |
| Always-available | not stated | (absent) | match |
| Re-enterable | not stated | (absent) | match |
| Personal 1 | +1 Str | STR +1 | match |
| Personal 2 | +1 Dex | DEX +1 | match |
| Personal 3 | +1 End | END +1 | match |
| Personal 4 | Athletics | Athletics | match |
| Personal 5 | Melee Combat | Melee Combat | match |
| Personal 6 | Vehicle | Vehicle | match |
| Service 1 | Mechanics | Mechanics | match |
| Service 2 | Gun Combat | Gun Combat | match |
| Service 3 | Gunnery | Gunnery | match |
| Service 4 | Melee Combat | Melee Combat | match |
| Service 5 | Recon | Recon | match |
| Service 6 | Battle Dress | Battle Dress | match |
| Specialist 1 | Comms | Comms | match |
| Specialist 2 | Demolitions | Demolitions | match |
| Specialist 3 | Gun Combat | Gun Combat | match |
| Specialist 4 | Melee Combat | Melee Combat | match |
| Specialist 5 | Survival | Survival | match |
| Specialist 6 | Vehicle | Vehicle | match |
| Adv. Education 1 | Advocate | Advocate | match |
| Adv. Education 2 | Computer | Computer | match |
| Adv. Education 3 | Jack o' Trades | Jack-of-All-Trades | match (canonical form, FR-017a normalization) |
| Adv. Education 4 | Medicine | Medicine | match |
| Adv. Education 5 | Leadership | Leadership | match |
| Adv. Education 6 | Tactics | Tactics | match |
| Rank 0 title | Private | Private | match |
| Rank 0 grant | Gun Combat-1 | Gun Combat 1 | match |
| Rank 1 title | Lieutenant | Lieutenant | match |
| Rank 1 grant | (none) | (absent) | match |
| Rank 2 title | Captain | Captain | match |
| Rank 2 grant | (none) | (absent) | match |
| Rank 3 title | Major | Major | match |
| Rank 3 grant | Leadership-1 | Leadership 1 | match |
| Rank 4 title | Lt Colonel | Lt Colonel | match |
| Rank 4 grant | (none) | (absent) | match |
| Rank 5 title | Colonel | Colonel | match |
| Rank 5 grant | (none) | (absent) | match |
| Rank 6 title | General | General | match |
| Rank 6 grant | (none) | (absent) | match |
| Cash row 1 | 1000 | 1000 | match |
| Cash row 2 | 5000 | 5000 | match |
| Cash row 3 | 10000 | 10000 | match |
| Cash row 4 | 10000 | 10000 | match |
| Cash row 5 | 20000 | 20000 | match |
| Cash row 6 | 50000 | 50000 | match |
| Cash row 7 | 50000 | 50000 | match |
| Material 1 | Low Passage | Low Passage | match |
| Material 2 | +1 Int | INT +1 | match |
| Material 3 | Weapon | Weapon | match |
| Material 4 | Mid Passage | Mid Passage | match |
| Material 5 | Weapon | Weapon | match |
| Material 6 | High Passage | High Passage | match |
| Material 7 | +1 Soc | SOC +1 | match |

## Notes

This career was already fully reconciled against the source earlier in this feature's Phase 6c
(the original transcription's throws, skill tables, and mustering-out tables all needed
correcting then); this pass re-fetched the source independently and found the reconciled file to
already match in full, with no further corrections or deviations.
