<!-- Open Game Content per OGL 1.0a; see LICENSE-OGL.txt -->

# Verification: Diplomat

Source-first re-read against https://evolvedexperiment.github.io/cepheus-srd/character-creation.html,
independent of this feature's transcription pass, per spec.md FR-023.

| Field | Source | Committed (`diplomat.toml`) | Verdict |
|---|---|---|---|
| Display name | Diplomat | Diplomat | match |
| Qualification | Soc 6+ | SOC 6+ | match |
| Survival | Edu 5+ | EDU 5+ | match |
| Commission | Int 7+ | INT 7+ | match |
| Advancement (promotion) | Soc 7+ | SOC 7+ | match |
| Re-enlistment | 5+ | 5+ | match |
| Advanced-education gate | Edu 8+ | EDU 8+ | match |
| Medical tier | 50%/75%/100% bracket ("professional"), per the "Table: Medical Bills" row listing Agent, Athlete, Bureaucrat, Diplomat, Entertainer, Hunter, Mercenary, Merchant, Noble, Physician, Pirate, Scientist, Technician | professional | match |
| Always-available | not stated | absent (false) | match |
| Re-enterable | not stated | absent (false) | match |
| Personal row 1 | +1 Dex | DEX +1 | match |
| Personal row 2 | +1 End | END +1 | match |
| Personal row 3 | +1 Int | INT +1 | match |
| Personal row 4 | +1 Edu | EDU +1 | match |
| Personal row 5 | Athletics | Athletics | match |
| Personal row 6 | Carousing | Carousing | match |
| Service row 1 | Admin | Admin | match |
| Service row 2 | Computer | Computer | match |
| Service row 3 | Carousing | Carousing | match |
| Service row 4 | Bribery | Bribery | match |
| Service row 5 | Liaison | Liaison | match |
| Service row 6 | Vehicle | Vehicle | match |
| Specialist row 1 | Carousing | Carousing | match |
| Specialist row 2 | Linguistics | Linguistics | match |
| Specialist row 3 | Bribery | Bribery | match |
| Specialist row 4 | Liaison | Liaison | match |
| Specialist row 5 | Steward | Steward | match |
| Specialist row 6 | Vehicle | Vehicle | match |
| Advanced-education row 1 | Advocate | Advocate | match |
| Advanced-education row 2 | Computer | Computer | match |
| Advanced-education row 3 | Liaison | Liaison | match |
| Advanced-education row 4 | Linguistics | Linguistics | match |
| Advanced-education row 5 | Medicine | Medicine | match |
| Advanced-education row 6 | Leadership | Leadership | match |
| Rank 0 title | Attaché | Attaché | match |
| Rank 0 grant | Liaison-1 | Liaison 1 | match |
| Rank 1 title | Third Secretary | Third Secretary | match |
| Rank 1 grant | (none) | (none) | match |
| Rank 2 title | Second Secretary | Second Secretary | match |
| Rank 2 grant | (none) | (none) | match |
| Rank 3 title | First Secretary | First Secretary | match |
| Rank 3 grant | Admin-1 | Admin 1 | match |
| Rank 4 title | Counselor | Counselor | match |
| Rank 4 grant | (none) | (none) | match |
| Rank 5 title | Minister | Minister | match |
| Rank 5 grant | (none) | (none) | match |
| Rank 6 title | Ambassador | Ambassador | match |
| Rank 6 grant | (none) | (none) | match |
| Cash row 1 | 1,000 | 1000 | match |
| Cash row 2 | 5,000 | 5000 | match |
| Cash row 3 | 10,000 | 10000 | match |
| Cash row 4 | 20,000 | 20000 | match |
| Cash row 5 | 20,000 | 20000 | match |
| Cash row 6 | 50,000 | 50000 | match |
| Cash row 7 | 100,000 | 100000 | match |
| Material row 1 | Low Passage | Low Passage | match |
| Material row 2 | +1 Edu | EDU +1 | match |
| Material row 3 | Mid Passage | Mid Passage | match |
| Material row 4 | High Passage | High Passage | match |
| Material row 5 | +1 Soc | SOC +1 | match |
| Material row 6 | High Passage | High Passage | match |
| Material row 7 | Explorers' Society | Explorers' Society | match |

## Notes

No discrepancies found. Every field of the committed file matches a fresh, independent read of the
source.
