...
| `throws.re-enlistment.characteristic` | string | no | Honored exactly as it is on the other four throws: a declared characteristic's modifier is added to the total and itemized in the recorded step (T195). The shipped example above omits it, which is why re-enlistment is thrown with no characteristic modifier by default — the same absence-means-none convention `throws.re-enlistment` already uses for `characteristic` in the worked example, not a rule that this position is special. |
...
| `ladders[].role` | string | **yes** | **New.** `"entry"` or `"commissioned"` (FR-007b). Exactly one ladder carries `entry`; at most one carries `commissioned`, and a career declaring `throws.commission` MUST declare one. A commission moves the character to the commissioned ladder at the lowest rank it declares. Without this field "the officer ladder is the second one listed" is a rule held in engine code. The `entry` ladder MUST declare rank 0: it is where the rank-zero bonus every career entry grants (FR-007) is looked up, and a ladder starting above rank 0 has nothing there for the walk to find (T182). |
...
| `rows[].range` | string | yes | `N`, `N-M`, or `N+`. May be negative. Exactly one row unbounded above, and it must sort highest: no row's range may overlap another's, and no row may declare a minimum above the unbounded row's — either is rejected at load rather than resolved by TOML file order the way the aging lookup's first-match scan would otherwise resolve it (T210). The **lowest** row is a floor: a modified result below it reads that row. Gaps between the rest of the rows are permitted and nothing requires their absence — the worked example above is not itself runnable as an override, since it has one, from `-5` to `-1` between its `"-6"` and `"0"` rows — and a modified result that falls in one fails the run rather than being silently read off whichever row sorts highest below it (T186, T205). |
...
| `*[].effects[].kind` | string | yes | For a `mishaps` row: one of `characteristic-class`, `debt`, `years`, `forfeit-career-benefits`, `roll-injury`. For an `injuries` row: `characteristic-class` only — nothing reads an injury row for any of the other four, so a correctly spelled one there validated clean and silently did nothing (T185). Either way a closed set, so a misspelling is caught rather than becoming a new effect nothing performs. `forfeit-term-benefit` was dropped (Phase 8 T145): every mishap-ended term forfeits its own benefit roll unconditionally in engine code (FR-020), so a row-level effect for the same thing could only ever double it. |
...
| `mustering-out.roll` | The die each benefit table is read with. Enumerated in FR-038 alongside the characteristic roll, because unlike a universal table's own die it is not declared by the table it reads: the cash and benefit tables live in each career's file and the die that reads them spans careers. A career's `mustering-out.cash` and `.benefits` arrays MUST cover the full range this roll plus the largest `retired-cash-dm` or `material-rank-dm` in force can produce; a total outside that range is a data problem reported when it is read, not clamped into the table's last row (T187, same treatment as `contracts/data-files.md`'s other positional table reads). |
