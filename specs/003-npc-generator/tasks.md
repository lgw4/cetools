...
### Phase 2D: The name tables (`names.py`)
...
### Phase 2G: The shipped data files
...
- [ ] T039 [P] Author `src/cetools/data/chargen/draft.toml` (OGC designation, `1d6`, six careers in row order, career-table spellings not the source's parenthetical aliases — research R6)
...
- [ ] T044 [P] Author `src/cetools/data/chargen/chargen-parameters.toml` (OGC designation; every key enumerated in `contracts/data-files.md`; not named `parameters.toml`)
...
- [ ] T048 Bring `src/cetools/data/careers/navy.toml` to career schema v2: `medical-tier = "service"`, `ladders[].role`, `tables.advanced-education` required
...
- [ ] T056 [P] Author `src/cetools/data/names/given-names.toml` (GPL-3.0 designation, `source` recorded, at least sixty gender-neutral entries — FR-043b, FR-043e, FR-043i)
- [ ] T057 [P] Author `src/cetools/data/names/surnames-africa.toml` (GPL-3.0 designation, `region`, `source`, at least forty entries)
- [ ] T058 [P] Author `src/cetools/data/names/surnames-asia.toml` (same shape)
- [ ] T059 [P] Author `src/cetools/data/names/surnames-central-america.toml` (same shape)
- [ ] T060 [P] Author `src/cetools/data/names/surnames-europe.toml` (same shape)
- [ ] T061 [P] Author `src/cetools/data/names/surnames-north-america.toml` (same shape)
- [ ] T062 [P] Author `src/cetools/data/names/surnames-south-america.toml` (same shape)
- [ ] T063 [P] Author `src/cetools/data/names/surnames-indigenous.toml` (GPL-3.0 designation; every entry carries `people` — FR-043d)
- [ ] T064 Verify the shipped surname tables differ in size by enough that a weighting taken over names rather than over regions would put at least one region outside SC-019's band, and record the sizes in `tests/unit/test_names.py`
- [ ] T065 Extend `tests/integration/test_validate_cli.py` and `tests/guards/test_data_layout.py` for twenty-six files, unique basenames tree-wide, and the two new subdirectories traversing without `__init__.py`

### Phase 2H: Licensing — two designations that the checks can tell apart

- [ ] T066 [TEST] Add `tests/unit/test_licensing.py` cases for the GPL-3.0 designation constant: a shipped data file carries exactly one designation, both is a failure, neither is a failure (FR-042, SC-015)
- [ ] T067 Narrow the Section 15 game-data notice in `LICENSE-OGL.txt` from the data root to the OGC subtrees, keeping the `_NOTICE_PATH` parenthesized-path and `_NOTICE_SUFFIX` "every `.toml` file" shapes parseable, and update `SECTION_15_NOTICES[-1]` in `tests/conftest.py` in the same commit (research R9)
- [ ] T068 Add the second designation constant to `tests/conftest.py`, assembled from two adjacent bytes literals so the test source does not designate itself
- [ ] T069 Turn `_assert_shipped_rules_data` in `tests/guards/test_packaging.py` into an exactly-one-of-two check over every `.toml` in the wheel and the sdist (FR-042a)
- [ ] T070 Extend `_uncovered` in `tests/conftest.py` with its mirror: every GPL-designated file must be claimed by neither the notice's paths nor its suffix, so a name table drifting into an OGC directory fails
- [ ] T071 Add the SC-015a fail-ability siblings to `tests/unit/test_licensing.py`, extending `test_the_coverage_check_sees_a_designated_file_the_old_scan_missed`: an OGC file planted outside the covered subtrees must fail the coverage check, and a GPL-designated file planted inside them must fail the mirror, each unlinked in a `finally`
...
- [ ] T073 [P] Update the licensing section of `README.md` and the "Licensing, which is not optional" section of `CONTRIBUTING.md`, both of which state that every `.toml` under `src/cetools/data/` is OGC and both of which become false the moment the first name table lands

### Phase 2I: The produced value (`character.py`)

- [ ] T074 [TEST] Write `tests/unit/test_character.py` for the seven types: field names and order per `contracts/library-api.md`, frozen and slotted, `HistoryStep.kind` and `StepEffect.kind` closed sets, `StepThrow.total == sum(faces) + modifiers`, a table-reading throw carrying `target = 0` and `success = True`
...
### Phase 2J: Public surface and guards
...
- [ ] T087 [P] [US1] [TEST] Add `tests/unit/test_generator.py` cases for mustering out: benefit count from terms and rank with the rank bonus **not** cumulative, the cash-roll cap, the material rank modifier (FR-016), the retired cash modifier applying exactly when the pension was qualified for (FR-017), and a pension earned in a **single** career with three terms in each of two careers paying nothing (FR-018)
...
- [ ] T090 [P] [US1] [TEST] Add `tests/unit/test_render_character.py` cases for FR-047c: a character titled by an earlier career and untitled by a later one keeps the earlier title, and no two ladders are ever compared
- [ ] T091 [US1] [TEST] Commit the golden reference files SC-009 requires under `tests/golden/`, written before the renderer exists and compared as bytes: `npc_titled.txt`, `npc_untitled.txt`, `npc_no_benefits.txt`, `npc_multi_career.txt`, `npc_titled_then_untitled.txt`, `npc_cascade.txt`
...
- [ ] T100 [US1] Implement mustering out, benefit rolls, the pension, medical bills, and ordered debt settlement in `src/cetools/generator.py`
...
- [ ] T105 [US1] Add the `npc` command to `src/cetools/cli.py` with `--seed`, `--name`, and `--rules-data`, drawing a seed from `secrets` when none is supplied (FR-053b), calling `generate_batch(..., count=1)`, writing the sheet to stdout and the seed/version/provenance to stderr via `_provenance_lines(indent=0)`, rejecting an empty or whitespace-only `--name` as a usage error (FR-053c)
- [ ] T106 [US1] Wire the golden comparisons in `tests/integration/test_golden.py` through `read_golden_bytes` against `stdout.encode("utf-8")` for the six npc references
...
- [ ] T114 [US2] [TEST] Commit `tests/golden/npc_full.txt` for a character carrying debt, pension, and history, compared as bytes
...
- [ ] T126 [US3] [TEST] Commit `tests/golden/npc_batch.txt` for SC-011: a multi-character batch whose bytes are exactly its sheets with one blank line between consecutive ones and no other text
- [ ] T127 [P] [US3] [TEST] Add the whole-set help assertion for `npc` to `tests/integration/test_cli.py`: `options_in_help(["npc"]) == {"--seed", "--count", "--name", "--rules-data", "--full", "--json", "--help"}`
...
- **Foundational (Phase 2)**: depends on Setup; **blocks every user story**
  - 2A structural must complete before any of 2B–2J, per Tidy First
  ...
  - 2H depends on 2G (the files it checks must exist)
  - 2I is independent of 2C–2H and can run alongside them
  - 2J depends on 2I (exports need the types)
...
- The `[TEST]` halves of 2C's six kinds (T018, T020, T022, T024) after T016 establishes the
  module
- All twenty-one data-file authoring tasks T039–T044 and T049–T063 — different files, no
  dependency on one another once the schemas exist
- T072, T073 in 2H; T078, T079 in 2J
- Every US1 test task T080–T090, T092, T093 — separate concerns, and T091's goldens are one
  file set
- The four sampled and property suites T107–T111
...
- 140 tasks: 3 Setup, 76 Foundational, 32 US1, 11 US2, 7 US3, 5 US4, 6 Polish
