...
- [ ] CHK031 - Is the deferral of Explorer's Society mechanics stated with an explicit rationale, not merely implied by "noted but not mechanically simulated"? [Completeness, Spec §Assumptions]
- [ ] CHK032 - Is the deferral of Courier Vessel mechanics stated with an explicit rationale? [Completeness, Spec §Assumptions]
- [ ] CHK033 - Is "recorded as a named benefit" specified precisely enough for the planner: what string or data structure represents Explorer's Society and Courier Vessel in the output? [Clarity, Spec §Assumptions]
- [ ] CHK034 - Is there a documented expansion condition for the draft table (e.g., "expands when a new career is implemented")? Is it clear enough to avoid a breaking data-structure change when the first new career is added? [Clarity, Spec §FR-010]
- [ ] CHK035 - Is it specified what happens if a future phase's draft table expansion produces a roll for an unimplemented career during this phase's runtime? [Gap, Spec §FR-010]

---

## Constitution Compliance

- [ ] CHK036 - Does the spec demonstrate, not merely assert, that adding the Scout career satisfies Principle V (data-driven extensibility): is there a concrete statement that Scout requires zero engine logic changes, supported by naming the new data fields? [Constitution §V, Spec §FR-001]
- [ ] CHK037 - Is there an acceptance scenario that validates the engine independently of the CLI (e.g., SC-003 calls the engine directly)? Does it cover the Scout data structure producing a valid character without any CLI invocation? [Constitution §II, Spec §SC-003]
...
- [ ] CHK039 - Are test-first requirements explicit: does the spec require tests to be written before implementation for each new function (qualification re-roll, draft table, Scout skill tables)? [Constitution §IV, Spec §SC-004]

---

## Edge Case Coverage

- [ ] CHK040 - Are requirements defined for a Scout who mustering outs after exactly one term (basic training + two skill rolls + one benefit roll only)? [Completeness, Spec §Edge Cases]
- [ ] CHK041 - Is the interaction between Education < 8 and the skill roll table selection process (only three tables available) specified as a requirement, not only as an edge case note? [Completeness, Spec §Edge Cases]
- [ ] CHK042 - Is the cash cap (3 rolls maximum) specified as applying per character overall or per career term? Is this consistent with the Navy implementation? [Clarity, Spec §US1-Scenario 5]
- [ ] CHK043 - Are requirements defined for the natural-12 mandatory extra term: what happens if the character would exceed the 7-term cap on a natural-12 re-enlistment? [Completeness, Spec §US1-Scenario 6]
