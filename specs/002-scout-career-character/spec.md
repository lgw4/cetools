...
- **FR-001**: The system MUST implement the Scout career as a data structure. The generation algorithm MUST require zero changes to process it. "Zero algorithm changes" means the per-career term-processing logic in `generate_character` and its helpers is untouched for Scout. The Career dataclass MAY gain new fields (e.g., a ranks table); adding a field to the data schema is not a generation algorithm change. New engine functions or modules added to support `--career` re-rolling (e.g., a wrapper entry point) are explicitly permitted and do not constitute algorithm changes.
...
- **FR-010**: The draft table for this phase MUST cover only the two implemented careers and MUST be represented as a `tuple[str, ...]` of 6 career identifiers indexed by `(roll - 1)`. Scouts (draft result 5 per SRD) and Navy (all other results) are the valid outcomes: `("navy", "navy", "navy", "navy", "scout", "navy")`.
...
- **FR-012**: When an unrecognized career name is passed to `--career`, the CLI MUST exit with code 1 and print the following message to stderr: `Unknown career '{name}'. Valid careers: navy, scout` (where `{name}` is the value supplied by the user). The complete exit code contract is: code 0 for all successes (character generated); code 1 for all user-facing failures (character death, unrecognized career name). No other exit codes are used.
...
- The SRD pension rule applies to all careers including Scout; pension calculation is deferred to a future phase (see FR-013).
