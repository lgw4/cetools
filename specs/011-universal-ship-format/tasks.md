...
- [ ] T013 Confirm FR-032 by running `uv run pytest tests/test_ship_builder.py tests/test_ship_generator.py tests/test_ship_design.py tests/test_ship_models.py tests/test_ship_tables.py` and verifying that every pre-existing assertion about a computed ship value still passes, unweakened and unretargeted. The only permitted edits to existing tests are the three named in T008, T009 and T011 — the `min_tl` → `tl` rename, the `CONFIG_MODIFIERS` → `CONFIGURATIONS` retarget, and the two `Ship(...)` sites gaining `tech_level` — plus the new coverage added by T002–T006 (SC-005).
