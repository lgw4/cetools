...
- [ ] T049 [P] Update `CONTEXT.md`: retire the "ship sheet" vocabulary in favour of "ship description", and retire the now-false claim at line 128 that armor and computer tech levels are "checked nowhere, since v1 has no tech-level model" — this feature builds that model, and `scripts/check_docs.py` cannot catch a prose claim that has merely gone stale. That same sentence backticks `` `ArmorRow.min_tl` ``, which T008 renames to `ArmorRow.tl`; the backticked symbol must be updated or `scripts/check_docs.py` fails at T050 on a symbol that no longer exists.
