...
- [ ] T015 [US1] Add a slot-order assertion to `tests/test_ship_description.py`: for the fully-equipped fixture, the sixteen sentence **slots** appear in the FR-004 order and in no other. Assert order over the slots, not over a sentence count — slot 8 emits the installed-weapons sentence plus one sentence per ammunition group, so the fully-equipped fixture carries more than sixteen sentences (FR-004, SC-004; contract §8, data-model.md §6).
