...
An armor option is a coating on the *hull*, not on a layer of armor. The SRD prices all three per ton of hull and lets reflec and stealth be added only once, so a ship carries one set of coatings however many layers of armor it has, and pays for them once. They live at the top level of a design file, as `armor_options = ["reflec"]`, rather than inside an `[[armor]]` table; a file written the old way is refused by name, and the fix is to move the key. They do need armor beneath them: the SRD introduces them as options "added to a ship's armor", so a coating on a bare hull is refused. That reads oddly next to the per-hull-ton price, and it is what the rules say.

A streamlined hull already has fuel scoops, because streamlining includes them and the 10% hull surcharge has paid for them. So the fitting question stops offering scoops once a streamlined configuration is pinned, and a design that names them anyway—hand-authored, or pinned before the configuration was drawn—is built with the scoops it has and charged nothing further. They are not listed among its special features either, since they are part of the hull rather than an addition to it. A *distributed* hull drops them from the same list for the opposite reason: it cannot mount scoops at all. Only the offer is withdrawn, though. Scoops typed at that prompt anyway are still accepted, and a distributed hull's refusal arrives when the ship is assembled, in the words of the rule that forbids it.
