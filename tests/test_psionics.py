...
def test_psi_strength_is_two_d6_minus_terms() -> None:
    # Gate roll 11 passes; Psi roll (9) minus 3 terms = 6.
    psi, _ = roll_psionics(terms_served=3, roller=SequenceRoller([11, 9], default=2))
    assert psi == 6


def test_psi_strength_floors_at_zero_and_skips_training() -> None:
    # Gate roll 11 passes; Psi roll 7 - 10 = -3, floored to 0. The high follow-up
    # values (12) must never be consumed, proving no talent checks are attempted
    # when Psi < 1.
    psi, talents = roll_psionics(
        terms_served=10, roller=SequenceRoller([11, 7, 12, 12, 12, 12, 12])
    )
    assert psi == 0
    assert talents == {}
...
def test_training_order_and_cumulative_penalty() -> None:
    # Gate roll 11 passes. Psi roll 7, terms 0 -> Psi 7 (PsiDM 0). Then five talent
    # checks each roll 8 (training target is still 8):
    ...
    psi, talents = roll_psionics(terms_served=0, roller=SequenceRoller([11, 7, 8, 8, 8, 8, 8]))
    assert psi == 7
    assert talents == {"Telepathy": 0, "Clairvoyance": 0, "Telekinesis": 0}


def test_learned_talents_are_level_zero() -> None:
    _, talents = roll_psionics(terms_served=0, roller=SequenceRoller([11, 7, 8, 8, 8, 8, 8]))
    assert all(level == 0 for level in talents.values())


def test_eligibility_gate_failure_skips_all_rolls() -> None:
    # Gate roll 10 (< 11) fails: return (0, {}) immediately. The high follow-up
    ...
    psi, talents = roll_psionics(
        terms_served=0, roller=SequenceRoller([10, 12, 12, 12, 12, 12, 12])
    )
    assert psi == 0
    assert talents == {}
...
def test_eligibility_gate_boundary_eleven_passes() -> None:
    ...
    # and same follow-up draw (12); gate 11 passes so the Psi roll (12) applies:
    # 12 - 3 = 9. (Gate 10 with the identical follow-up yields Psi 0 there.)
    psi, _ = roll_psionics(terms_served=3, roller=SequenceRoller([11, 12], default=2))
    assert psi == 9


def test_eligibility_gate_boundary_ten_fails() -> None:
    # Gate roll 10 fails even though the next draw (12) would give a high Psi.
    psi, talents = roll_psionics(terms_served=3, roller=SequenceRoller([10, 12], default=2))
    assert psi == 0
    assert talents == {}
